...
#let article(
  ...
  // ---- tables ---------------------------------------------------------------
  // align: (left + horizon) overrides whatever alignment markers the qmd
  // source declares (:---:, ---:, etc.), so every table renders uniformly:
  // cells left-aligned horizontally, vertically centered. Headers inherit
  // the same alignment plus the bold-blue treatment below.
  set table(
