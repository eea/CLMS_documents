...
original-filename: guidelines/editor-manual_v1.qmd
...
-   Click the **Render** button at the top of the RStudio window. ![Render button](editor-manual_v1-media/rstudio-render.png)

-   Or enable **Preview on Save** (in the RStudio IDE toolbar). This option automatically re-renders your document every time you save it. ![Render on Save](editor-manual_v1-media/rstudio-render-on-save.png)
...
![](editor-manual_v1-media/rstudio-render-viewer-tab.png)
...
![](editor-manual_v1-media/rstudio-render-viewer-options.png){height="300px"}
...
![](editor-manual_v1-media/rstudio-render-docx.png)
...
3. Upon review approval:
    - The PR is merged.
    - The final, official version is automatically rendered and published at [https://library.land.copernicus.eu/](https://https://library.land.copernicus.eu)
...
When documents are merged into the `main` branch:
 ...
 - Based on the scope and nature of changes, it assigns the next appropriate version number
 - Minor changes (typos, small clarifications) increment the patch version (e.g., 2.1.0 → 2.1.1)
 - New content or features increment the minor version (e.g., 2.1.0 → 2.2.0)
 - Breaking changes or major rewrites increment the major version (e.g., 2.1.0 → 3.0.0, which may also update the filename from v2 to v3)

The major version number in your document filename (e.g., `document_name_v2.qmd`) represents the base major version. The complete version tracking is managed automatically by the Technical Library system.
...
You accidentally removed an important section from your local file. Restore the latest published version to get it back, then continue editing.

**Starting from a previous version:**
Version 2.2.0 introduced problems. Restore version 2.0.0, make corrections, and publish. When the library is released, your changes will be assigned a new version number based on their scope.

**Bringing back a deleted file:**
A file was removed from your project but still exists in the Technical Library. Restore it to recreate the file locally, then you can update and republish it.
...
2. **Edit:** Make changes locally (file is still named `CLMS_HRL-Forest_PUM_v2.qmd`)
3. **Publish to develop:** Push changes with `git docs-publish` - your document appears in the develop environment for preview
4. **Release process:** When the Technical Library is released (develop → test → main), AI analyzes your changes and assigns the next version number
5. **Result:** Your edited document appears in the Technical Library with a new version (e.g., 2.3.0 for new features, 2.2.1 for fixes, or 3.0.0 for breaking changes)

Note that version numbers are assigned only during the final release, not when you push to develop. See the "Releasing Official Documentation" section for details on how versioning works.
