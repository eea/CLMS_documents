...
title: High Resolution Layer Small Landscape Features 2021 – Algorithm Theoretical
  Basis Document (ATBD)
toc: true
toc-depth: 3
toc-title: Content
...
@fig-figure1 below shows an overview of the workflow, which can be sub-divided in 3 main steps:
...
![HRL SLF 2021 workflow](CLMS_ATBD_HRLSLF_v1.1-media/img-19af4590dd266c8c9cb96e9eebf8f88e30ed117f.png){width="6.27in" #fig-figure1}
...
|---------------------------------------------------|-------------------------------------------------|----------------------------------------------------------------------------------------------------|
|CLMS SWF 2018|Training|[https://land.copernicus.eu/](https://land.copernicus.eu/)|
|CLMS VLCC TCD 2021|Training + masking|[https://land.copernicus.eu/](https://land.copernicus.eu/)|
|CLMS VLCC CTY 2021|Masking|[https://land.copernicus.eu/](https://land.copernicus.eu/)|
|CLMS Urban Atlas 2021|Masking|[https://land.copernicus.eu/](https://land.copernicus.eu/)|
...
|CLMS CLCplus Backbone 2021|Training|[https://land.copernicus.eu/](https://land.copernicus.eu/)|
...
![Urban Mask production steps](CLMS_ATBD_HRLSLF_v1.1-media/img-f1a2f36404cd309a9239b2a29b76a6fc4622a636.png){width="4.73in" #fig-figure2}
...
To maximize the applicability of this reference dataset and foster economies of scale for its generation, the selection of samples was performed automatically, based on a stratified approach considering (see @fig-figure3): 

- Environmental zones to reflect the spectrally heterogeneous coverage of woody  vegetation throughout Europe (@fig-figure3 Top)

- Heterogeneity of input datasets (i.e. VHR_IMAGE_2021 dataset contains imagery from different sensors) (@fig-figure3 Centre)

- Urban and rural areas (@fig-figure3 Bottom)
...
![External dataset used for Reference Database sampling stratification](CLMS_ATBD_HRLSLF_v1.1-media/img-cdea45cb195629a1bdac80dbd5adc2aab5f6a896.png){width="5.81in" #fig-figure3}

![Reference Database samples distribution](CLMS_ATBD_HRLSLF_v1.1-media/img-0aeffbc120bdc6740e4085604ad2c7f3d15b624c.png){width="6.27in" #fig-figure4}
...
An example of a reference database sample is illustrated in @fig-figure5 below. One the left, the  initial vector result of visual and manual enhancement. On the right, the rasterized version ready  to be ingested in the training process.

![Reference database patch example](CLMS_ATBD_HRLSLF_v1.1-media/img-7f7921a5b76c664e0971072cce1e96907da340a1.png){width="5.34in" #fig-figure5}
...
![Model architecture of SATLASNET. Separate heads can be used to predict outputs of different formats or different thematic nature (source: Bastani et al, 2023)](CLMS_ATBD_HRLSLF_v1.1-media/img-6359dfdbf09c541f640c3720b6a554f4f2bf2223.png){width="6.27in" #fig-figure6}

Of the components shown in @fig-figure6, the backbone + FPN of the aerial pre-trained model (0.5-2m/px high-res imagery, Single-image, RGB) were used as a basis to take advantage of the pre-trained model, then the segmentation prediction head was attached and fine-tuned to woody vegetation detection using the reference database.
...
**Erosion and Dilation**: Table 4 @fig-figure8 Using the width threshold defined in the product specification (Table 4), an  erosion operation was performed (using half the with) followed by dilation. This sequence  removed parts from the raster which are too thin to “survive” the erosion operation and was key to distinguish linear features from patchy ones (refer to @fig-figure7 for visual representation).  This distinction is done by overlaying the processed raster with the original input which allowed for the classification of each pixel into linear and patchy feature candidates.

![An erosion followed by a dilation operation allows to distinguish between linear and patchy features in the raster](CLMS_ATBD_HRLSLF_v1.1-media/img-aa134eba6cce3b395453b7fc397b281b18a661be.png){width="6.21in" #fig-figure8}
...
- Class 6: Features that were connected to valid Geometries (Classes 1 and 2) and were covered by crops (according to the threshold of 80%)
  ...
  3. For the final product, Classes 1, 2 and 5 were extracted from the intermediate product (see @fig-figure9 below).

![Identified polygons in the intermediate product. The image shows an area close to Merano, Italy and demonstrates the identification of agricultural areas.](CLMS_ATBD_HRLSLF_v1.1-media/img-767c0ee584714b1b05aa33bef49f66b0a83d5175.png){width="6.27in" #fig-figure9}
...
- Agricultural areas are defined as features with more than 80% coverage by agriculture, according to the agricultural mask (see @sec-pre-processing)
...
![Example of the random distribution of SSUs within a PSU](CLMS_ATBD_HRLSLF_v1.1-media/img-a9079df225d0f0e930a75fb45cb4dd8fd3583733.png){width="2.63in" #fig-figure10}
...
[^2]:[Copernicus Land Monitoring Service – Service desk](https://land.copernicus.eu/en/contact-service-helpdesk)
