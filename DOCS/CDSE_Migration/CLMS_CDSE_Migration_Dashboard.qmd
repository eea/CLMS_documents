...
onboardingPlot = {
  ...
  function buildChart(container) {
    ...
    const rawMax = Math.max(...data.map(d => Math.abs(d.value)), 1)

    // Sanity cap: guard against transient OData=0 days producing ~10M
    // outlier deltas. Use rolling median of all absolute deltas;
    // cap at 5x the median so a single API failure day doesn't
    // blow out the y-axis scale and hide real daily changes (~50K).
    const values = data.map(d => Math.abs(d.value));
    const sorted = [...values].sort((a, b) => a - b);
    const mid = Math.floor(sorted.length / 2);
    const rollingMedian = sorted.length % 2 === 0
      ? (sorted[mid - 1] + sorted[mid]) / 2
      : sorted[mid];
    const maxVal = Math.min(rawMax, Math.max(rollingMedian * 5, 1));
...
${changes.length === 0 ? htl.html `<div style="background:#f5f5f5;border:1px solid #ddd;border-radius:6px;padding:12px;margin:12px 0;color:#888">No new CLMS products were onboarded this week.</div>` : ""}
...
<div style="background:#fff3e0;border:1px solid #ffcc80;border-radius:6px;padding:12px;margin:12px 0">
  <strong>📦 File Level Statistics</strong>
