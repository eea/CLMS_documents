...
- A separate technical class has been introduced for the seawater areas in the buffer  around the production AOI since the 2021 production (see @sec-product-methodology-and-workflow and AD-1).
...
# Review of user requirements {#sec-review-of-user-requirements}
...
# Product application areas and exemplary use cases {#sec-product-application-areas-and-examplary-use-cases}
...
# Product description {#sec-production-description}

The CLCplus Backbone 2023 product comprises an updated layer of the main land cover raster layer, as well as three auxiliary layers (see @fig-figure1). 

![Overview of CLCplus Backbone layers produced for the reference year 2023](2023_PUM_v1-media/img-3ae7a950b89b88ab5164a45a96c174e993f664d9.png){width="3.08in" #fig-figure1}
...
![CLCplus Backbone 2023 for tile E44N36 (Copenhagen)](2023_PUM_v1-media/img-66b2331d63fc430e212e30aa8999c547d0b8dc82.png){width="6.27in" #fig-figure3}

@fig-figure3 displays a single tile (E44N36) of the CLCplus Backbone 2023, showing the area around Copenhagen. Besides the tiles for the EEA38+UK extent, the CLCplus Backbone 2023 is available in the national projections for the French DOMs.
...
|  |  |
|--|--|
|Prefix: CLMS|SpatialResolution (SpatialResolutionUnit): 10m||Prefix: CLMS|SpatialResolution (SpatialResolutionUnit): 10m|
...
Most of the class definitions for the CLCplus Backbone 2023 comprise a 50% area threshold to  express that the dominant land cover should be assigned. While this is a plausible approach  there are many situations where the land cover within a single 10m pixel comprises a spatial mix  of different classes and no single class reaches an absolute majority of 50%. A simple example is  provided in @fig-figure4. 

![Exemplary 10x10m plot illustration of a situation where no single land cover class  covers 50% of the area](2023_PUM_v1-media/img-0f13569c926a8fcffb56ce87a86f6969e7efe854.png){width="2.63in" #fig-figure4}

To address this issue, a relative majority is used in most cases for the class assignment. This logic is implemented in detail through a decision tree approach (see @fig-figure6) which does not only  define the area threshold but further clarifies and limits for each class the EAGLE Land Cover  Component (see @fig-figure5) and EAGLE Land Characteristics (LCH, where needed) considered with  the evaluation of the area threshold. The approach is detailed in section @sec-decision-tree-for-area-thresholds. In general, the  area fractions of different land cover classes are to be understood as what can be reasonably  evaluated in HR and VHR remote sensing imagery at nadir. 

![Levels 1-5 of the EAGLE Land Cover Components (LCC) v3.21. The hierarchical  levels allow for resolving the LCC assignment in the most suitable way. In case level 1 is too  general, the most suitable lower level is chosen](2023_PUM_v1-media/img-eeda59c50bb28bb4c441bd411b0a293d236bdfac.png){width="6.27in" #fig-figure5}
...
Example 1 – Hypothetical example, considering the mix which was already presented in @fig-figure4:

![](2023_PUM_v1-media/Example1.png){width="5.73in"}

Example 2 – Real world example extracted from @fig-figure7 (Row 2, Column 5):

![](2023_PUM_v1-media/Example2.png){width="5.73in"}
...
![CLCplus Backbone Data Score Layer (RASDL) of the reference year 2023 (±3 months)](2023_PUM_v1-media/img-957b567d0ef07f50f054c5cee41babd5cf0d641d.png){width="6.27in" #fig-figure17}

The clearly visible stripe patterns (see @fig-figure17) from SW to NE correctly represent the different  data quantities between the overlapping areas of the S-2 swaths. Besides this normal general  pattern of the RASDL, there are some patterns that are worth further explaining: S-2 GRANULE overlaps are often visible as horizontal and vertical stripes with slightly higher observation  counts. This is because there is a higher chance in the overlap areas that one of the GRANULES fulfils the cloud cover threshold used in scene selection.
...
The RASPL for the CLCplus Backbone 2023 is shown in @fig-figure21. The green areas represent these  re-coded and thus improved areas. The post-processing rulesets were intentionally tailored to  be rather conservative and introduce adaptations only where auxiliary layers provide very  reliable information. This is reflected in the very small fraction of pixels that underwent  automated corrections (0.03%) during the post-processing.

![CLCplus Backbone Raster Post-Processing Layer (RASPL) 2023](2023_PUM_v1-media/img-873fc1d0b90c1e8fea175db5a26fb19ff0db0f39.png){width="6.27in" #fig-figure21}
...
|CLMS|Copernicus Land Monitoring Service|[https://land.copernicus.eu/en](https://land.copernicus.eu/en)|
...
|EEA38|The 32 member and 6 cooperating countries of the EEA|[https://land.copernicus.eu/en/faq/general-questions/what-is-eea38](https://land.copernicus.eu/en/faq/general-questions/what-is-eea38)|
...
|INSPIRE|Infrastructure for Spatial Information in Europe|[https://knowledge-base.inspire.ec.europa.eu/index_en](https://knowledge-base.inspire.ec.europa.eu/index_en)|
...
![Colour palette for the Raster (RAS) Product](2023_PUM_v1-media/RasterProduct.png){width="6.19in"}

### Data Score Layer (RASDL)

![Colour palette for the Data Score Layer](2023_PUM_v1-media/DataScoreLayer.png){width="3.69in"}

### Raster Confidence Layer (RASCL)

![Colour palette for the Raster Confidence Layer](2023_PUM_v1-media/RasterConfidenceLayer.png){width="3.69in"}

### Post-Processing Layer (RASPL)

![Colour palette for the Raster Post-processing Layer](2023_PUM_v1-media/RasterPost-ProcessingLayer.png){width="5.78in"}
...
# Document history {#sec-document-history}

|Version|Date|Short description of changes|
|---------------------------------------|-----------------------------------------------------|------------------------------------------------------------------------------------------------------------|
|1.2|01.04.2025|Initial published issue|

# Applicable documents {#sec-applicable-documents}
