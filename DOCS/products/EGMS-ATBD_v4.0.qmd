...
![Figure 1: Comparison of two images from a radar and optical sensor acquired over the same area. Radar can penetrate cloud and acquire data independently of sun illumination. Radar images, however, are usually more difficult to interpret to non-specialists. On the left, an amplitude SAR image, where pixel brightness depends on the amount of electromagnetic energy backscattered towards the radar by the elementary scatterers present in each resolution cell.](EGMS-ATBD_v4.0-media/img-bffdb27be629080e3bca603b202695144aa031ef.png)
...
![Figure 2: An example of speckle, Tiber Valley, Italy (courtesy ESA)](EGMS-ATBD_v4.0-media/img-b56ac516fc1bf21d5dd681c41efa1f95175c71fc.png)
...
![Figure 3 Geometry of a typical spaceborne SAR system, defining range, ground range and azimuth directions (from Ferretti, 2014).](EGMS-ATBD_v4.0-media/img-4521bab106696ef550aef776fce0d03f7224d42f.png)
...
![Figure 5: Example of a radar (amplitude) image. To limit the inherent ‘speckle’ noise found in radar images, the amplitude values from 30 Sentinel-1 images acquired over the area of interest have been averaged.](EGMS-ATBD_v4.0-media/img-72b307a3275507e8c711f178c0cf2cbccae7f4f2.png)
...
![Figure 6 Example of electromagnetic wave polarization applied to remote sensing. From https://www.eorc.jaxa.jp/ALOS/en/img_up/pal_polarization.htm](EGMS-ATBD_v4.0-media/img-3d2b7b7a0055c64b4b5137b099503a8835c58f76.png)
...
![Figure 9 Formation of an interferogram.](EGMS-ATBD_v4.0-media/img-ee06080226d54216cc671f3421062bf214cbdbbf.png)
...
![Figure 10: Example of a SAR differential interferogram (after removal of topographic components). Interferometric fringes are generated by the co-seismic deformation pattern caused by the L’Aquila earthquake in Italy (April 2009). Images acquired by Cband SAR with approximate 5.6 cm wavelength, meaning each interferometric fringe represents approximately 28 mm displacement in the satellite line of sight. SAR data acquired by ESA’s Envisat mission.](EGMS-ATBD_v4.0-media/img-ec3e2f452b4b0b9706c5f1a34e8d814e3b86d37a.png)
...
![Figure 11: a) multi-interferogram techniques aim at identifying coherent radar targets (referred to as persistent scatterers) that are only slightly affected by phase decorrelation. b) Given a data stack of multi-temporal SAR acquisitions, the analysis is carried out on a sparse PS grid, rather than on all image pixels. c) After PS identification and reference point selection, atmospheric effects are estimated and removed taking advantage of their statistical behaviour. The APS superimposed on each SAR image is almost uncorrelated in time, but is correlated spatially, while displacement phase components are usually correlated in time. (Adapted from Ferretti, 2014).](EGMS-ATBD_v4.0-media/img-38aa13d27f492c8f8fb4080094a4cbfb100cc63f.png)
...
![Figure 12: Point scatterers (PS) vs distributed scatterers (DS).](EGMS-ATBD_v4.0-media/img-95f72dba73337d5ca5e616489165f7b3fb4d2947.png)
...
![Figure 14 Example of baseline plot where each acquisition corresponds to a point having a specific geometrical and temporal baseline with respect to a reference (primary) orbit.](EGMS-ATBD_v4.0-media/img-d3d81e9011d6885a67a1a484e1d8f6250a72b4a6.png)
...
![Figure 16 - Prats et al., 2012 - Example of along-track linear phase ramps in a TOPS interferogram due to an artificially induced azimuth constant registration error (middle). Interferogram coherence (left). The same interferogram without azimuth registration error (right).](EGMS-ATBD_v4.0-media/img-b375ca0fc8bc0ac11534438abbb5cb7461214894.png)
...
![Figure 17 Multi-interferogram algorithms aim at exploiting multi-temporal InSAR data stacks for separating different phase components: target displacement, topography dependent components and atmospheric effects.](EGMS-ATBD_v4.0-media/img-e15c4c691d9e2ebf8a44e65cf47187df74e440e1.png)
...
![Figure 18 Differential interferogram generation (adapted from Ferretti, 2014). In the original and the synthetic interferogram phase components related to the “flat Earth” model have been removed for visualization purposes.](EGMS-ATBD_v4.0-media/img-efcf468554be5c932a9731b510fd8d530309dfb3.png)
...
![Figure 22 : The radar signature of a dominant scatterer resembles the impulse response function of the system. Depending on the SCR value, one single PS (the red triangle) can create many other measurement points (orange triangles).](EGMS-ATBD_v4.0-media/img-b48f3532cf3234b08c5e6d24260f2a62d3c0f9a5.png)
...
![Figure 24 Example showing a phase unwrapping procedure based on a linear model for target motion. In (a) the true motion of the target (black samples) in not perfectly linear, but phase unwrapping based on the linear model can still provide the correct number of phase cycles, while in (b) the model (dashed red line) leads to underestimation of target motion and wrong estimation of phase cycles for several interferograms (red samples).](EGMS-ATBD_v4.0-media/img-561384cb2f14fae8ae8e818951620b0442f4b807.png)
...
![Figure 27 look and feel of the same data (a) following a standard reference approach and (b) the EGMS reference approach.](EGMS-ATBD_v4.0-media/img-77db72375a0b00ffb0e05e07ea08aa2d2b181b68.png)
...
![Figure 28 Single vs multi-burst analysis. By processing a single satellite burst (for instance, ‘Burst A' in the picture), memory requirements and computational times will be reduced compared to a multi-burst approach, jointly processing Burst A and B. However, the phase unwrapping of pixels in valleys v1, v2, and v3 with respect to the reference point P0 (the white triangle in the picture) would be extremely challenging as the density of coherent points would be quite low over the mountains.](EGMS-ATBD_v4.0-media/img-07e2633f80d17ca06867ac3999cb27f758efda27.png)
...
![Figure 29 Sentinel-1, IW acquisition mode. Each image is composed by 3 main swaths, each one being a collection of 9 bursts. https://sentinels.copernicus.eu/web/sentinel/user-guides/sentinel-1-sar/acquisition-modes/interferometric-wide-swath](EGMS-ATBD_v4.0-media/img-146ef7be7fbfd2e7fcf01908515d579e5ac065bf.png)
...
![Figure 30 Sentinel-1 constellation acquisition by relative orbit. https://sentinels.copernicus.eu/web/sentinel/missions/sentinel-1/satellite-description/orbit](EGMS-ATBD_v4.0-media/img-56f1e507b27f4af0b00456ca54d2820e567d3c84.png)
...
![Figure 31 Representation of Sentinel-1 orbital tube.](EGMS-ATBD_v4.0-media/img-7543d4612d3eb37d34f53162eb92646da49269cd.png)
...
![Figure 33 Map of European earthquake \>M=5.5 occurring between Feb 2015 and Dec 2020](EGMS-ATBD_v4.0-media/img-f5228b0f66823ade7dd45ecd43ed9800e316c4b2.png)
...
![Figure 34 Example of grid of Pseudo-PS and two sets of measurement points coming from two InSAR data-stacks.](EGMS-ATBD_v4.0-media/img-8b3302c2067f725a0566dc0dbacfb76bbcc00922.png)
...
![Figure 37 The same real displacement can result in different measured displacements is viewed from different angles.](EGMS-ATBD_v4.0-media/img-4fcb1b6dd078d100298289f68d14a0cb81e5edcd.png)
...
![Figure 38 Example of displacement decomposition by combining data from ascending and descending acquisition geometries.](EGMS-ATBD_v4.0-media/img-72bf46cffb32f8ac9181575602286d5e520dd575.png)
