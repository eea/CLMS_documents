...
This document serves as the Algorithm Theoretical Basis Document for the European Ground Motion Service (EGMS), a core component of the Copernicus Land Monitoring Service. It details the scientific and technical underpinnings of EGMS product generation, encompassing the entire process from basic Synthetic Aperture Radar (SAR) interferometry principles to the specifics of generating Basic (L2a), Calibrated (L2b), and Ortho (L3) InSAR products. The document outlines the theoretical framework, processing chains, and calibration methodologies employed, while also highlighting the consistency and homogeneity achieved through careful tuning and cross-check procedures across the independent InSAR processing chains contributing to EGMS deliverables.
...
![A vector diagram illustrating the relationship between signal, noise, and phase error in Interferometric Synthetic Aperture Radar (InSAR) measurements, accompanied by a mathematical condition for phase uncertainty. The diagram displays a green vector labelled 'A (signal)' originating from the left and pointing horizontally right. A red vector labelled 'n (noise)' originates from the tip of the 'A (signal)' vector and points upwards and slightly right, terminating at a red dot. A blue vector represents the sum of the signal and noise vectors, extending from the origin of 'A (signal)' to the red dot. The angle between the 'A (signal)' vector and the blue resultant vector is labelled 'φ'. A light grey circle is centered at the tip of the 'A (signal)' vector, encompassing the red 'n (noise)' vector, visually representing the noise contribution. The accompanying mathematical expression is: 'If SNR \>\> 1 → σ_φ ~ σ_n / µ = D_A'. This indicates that if the Signal-to-Noise Ratio (SNR) is much greater than 1, then the standard deviation of the phase (σ_φ) is approximately equal to the ratio of the standard deviation of the noise (σ_n) to the mean signal amplitude (µ), with this ratio also being denoted as D_A.](European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-f6d1c5f382a7c0caf91add557b6f3a361ef91189.png)
...
![This image is a 3D rendering of the Earth from space, illustrating satellite data acquisition coverage, likely for Interferometric Synthetic Aperture Radar (InSAR). The Earth, showing visible landmasses (primarily Africa, Europe, and parts of Asia) and oceans, is enveloped by numerous translucent, light-grey lines representing satellite orbital tracks or coverage swaths. Two small satellites are depicted in orbit, one positioned above the Northern Hemisphere and another below the Southern Hemisphere, each emitting a green, laser-like beam towards the Earth's surface, symbolising active InSAR data collection. In the bottom-right corner, a control panel is visible with the label 'Full coverage in days:'. Below this, a timeline with numbers '0 1 2 3 4 5 6' indicates the duration. A green highlighted bar extends from '0' to approximately '1', suggesting that full coverage is achieved within one day. This visualisation pertains to the processing strategies for the European Ground Motion Service (EGMS), which relies on InSAR data.](European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-56f1e507b27f4af0b00456ca54d2820e567d3c84.jpg)
...
![This diagram illustrates a data processing workflow for generating L2b and L3 products from L2a products and Global Navigation Satellite System (GNSS) model data, likely within the context of European Ground Motion Service (EGMS) Interferometric Synthetic Aperture Radar (InSAR) data calibration. The workflow begins with a \`GNSS Model\` which branches into two parallel processes: 1. \`GNSS Model Interpolation\`. 2. \`GNSS N/S Component in SAR LOS\` (Synthetic Aperture Radar Line Of Sight). The \`GNSS Model Interpolation\` output, along with \`L2a Products\`, feeds into \`Interpolated Model Projection\`. The output of \`Interpolated Model Projection\` then proceeds to \`SAR/GNSS Referencing\`. \`SAR/GNSS Referencing\` leads to a \`Compensation\` step, which also receives direct input from \`L2a Products\`. This first \`Compensation\` step produces \`L2b Products\`. The \`L2b Products\` then feed into a second \`Compensation\` step. This second \`Compensation\` step also receives input from the \`GNSS N/S Component in SAR LOS\` branch. The output of the second \`Compensation\` step feeds into \`Decomposition\`. Finally, \`Decomposition\` produces \`L3 Products\`, representing the end product of this processing chain.](European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-7be7b9e910835788e15dbcbd0ffa9ce75f835d7d.png)
