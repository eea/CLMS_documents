...
- [<span class="toc-section-number">1</span> Summary](#summary)
  - [<span class="toc-section-number">1.1</span> By Product
    Scope](#by-product-scope)
- [<span class="toc-section-number">2</span> Per-Group Ingestion
  Status](#per-group-ingestion-status)
- [<span class="toc-section-number">3</span> Groups Not Yet on CDSE
  (0%)](#groups-not-yet-on-cdse-0)
- [<span class="toc-section-number">4</span> Partially Ingested
  Groups](#partially-ingested-groups)
- [<span class="toc-section-number">5</span> Fully Ingested Groups
  (100%+)](#fully-ingested-groups-100)
- [<span class="toc-section-number">6</span> Scope
  Breakdown](#scope-breakdown)
  - [<span class="toc-section-number">6.1</span> 🌍 Global
    Products](#earth_africa-global-products)
  - [<span class="toc-section-number">6.2</span> 🇪🇺 Pan-European
    Products](#european_union-pan-european-products)
  - [<span class="toc-section-number">6.3</span> 📍 Priority Area
    Products](#round_pushpin-priority-area-products)
- [<span class="toc-section-number">7</span> Services](#services)
- [<span class="toc-section-number">8</span> Methodology](#methodology)

**Last updated:** 2026-07-14 12:00 UTC

This report tracks the migration of CLMS products from the
[land.copernicus.eu](https://land.copernicus.eu/en/dataset-catalog)
portfolio to the [Copernicus Data Space Ecosystem
(CDSE)](https://dataspace.copernicus.eu/). It is updated weekly.
...
The following product groups have no datasets ingested on CDSE yet.
These are the main gap in the migration.

- 🇪🇺 Pan-European — **HR Imperviousness** — 26 datasets pending
- 🌍 Global — **Vegetation Phenology and Productivity Parameters** — 16
  datasets pending
...
- 🇪🇺 Pan-European — **European Ground Motion Service** — 4 datasets
  pending
...
These groups are fully available on CDSE. Percentages above 100%
indicate CDSE contains additional derived datasets (e.g., confidence
layers) beyond the land portal’s base product listing.
...
Products with worldwide coverage — bio-geophysical variables
(temperature, water, snow, vegetation, energy fluxes), land cover
(Dynamic Land Cover, Water Extent), and vegetation parameters.

**15/17** groups have datasets on CDSE.

### 🇪🇺 Pan-European Products

Products covering the EEA39 region (Europe) — high-resolution land
cover, land use, forest, grassland, and imperviousness layers, thematic
products (CLC, CLC+, EU-Hydro, EGMS, Image Mosaic), and vegetation
...
Local-scale monitoring products covering specific areas of interest
(urban, coastal, riparian, protected areas, hot spots).
...
- [Sentinel
  Hub](https://documentation.dataspace.copernicus.eu/APIs/SentinelHub/Data/CLMS.html)
...
1.  **Fetches the land.copernicus.eu portfolio** — all 249 datasets
    across 39 product groups via the Plone REST API.
2.  **Crawls the CDSE CSV catalogue** — discovers dataset IDs available
    on S3 under `bio-geophysical`, `landcover_landuse`, and
    `land_cover_use_in_priority_areas`.
3.  **Queries the CDSE OData API** — total CLMS product count.
4.  **Cross-references** — maps CDSE dataset IDs to land portal product
    groups and reports ingestion status per group.
