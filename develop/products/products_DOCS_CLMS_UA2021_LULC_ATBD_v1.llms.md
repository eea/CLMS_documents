...
This Algorithm Theoretical Basis Document (ATBD) provides a detailed technical foundation for the Copernicus Urban Atlas 2021 product suite. It meticulously describes the algorithms, methodologies, and workflows employed to generate high-resolution land cover and land use data for European urban areas. Focusing on change detection between 2018 and 2021, the document details the integration of Very High Resolution and Sentinel-2 satellite imagery, advanced deep learning classification, and robust post-processing. Crucially, it elaborates on the innovative approach for classifying Green Urban Areas by accessibility and addresses comprehensive quality control and known technical challenges.
...
Copernicus Urban Atlas, Algorithm Theoretical Basis Document, land use/land cover mapping, change detection methodology, Sentinel-2 time series, Very High Resolution imagery, Principal Component Analysis, Root Mean Square Deviation, Green Urban Areas classification, Computer Assisted Photo Interpretation, thematic accuracy assessment, data fusion
...
- **Metro Strava** data was identified as another valuable data source that contributes to classification. Strava is a widely used digital platform that allows millions of users around the world to track their physical activities such as running, cycling, and walking via Global Positioning System (GPS)-enabled devices. The data generated through these activities, commonly referred to as Strava data, provides rich insights into human movement patterns, particularly in outdoor environments. Each activity recorded on Strava includes spatial tracks, time-stamped routes, distance, speed, elevation gain, and sometimes physiological data such as heart rate or power output, depending on user settings and devices. In the frame of this project, we assumed that roads, streets and paths that are used by Strava users for recreational activities have public access.  
  Although Strava had agreed to provide the data, the data was not provided for the full pan-European area. As a result, the data could not be used to comply with pan-European consistency requirements.
...
The complete Sentinel-2 archive for the specified reference year and tile was utilized for processing. A dedicated preprocessing routine was applied to Level-2A data across the entire time series to generate cloud-free percentile composites of the corresponding Sentinel-2 bands, along with a harmonic fitting of the time series. For further details, refer to [Section 3.3.2](#sec-pre-processing-methodology) below.
...
\\f(t)=\sum\_{i=0}^{P}p\_{i}t^{i}\\\\+\sum\_{k=0}^{H}(\alpha\_{k}\mathrm{sin}\left(k\omega t\right)+\beta\_{k}\mathrm{cos}\left(k\omega t\right))\\

in this equation, p_(i) are the coefficients of the polynomial term with order P, H is the order of the harmonic system and α_(k) and β_(k) are the coefficients of the harmonic functions with order k. \\\omega= 2π\\/year is the base angular frequency of the harmonic system. The coefficients p_(i), α_(k) and β_(k) are estimated by performing ordinary least squares fit to the signal of each pixel of the time series.  
Cloud and atmospheric artifacts ([Figure 2](#fig-figure2)) are mitigated through a weighting scheme based on the Sentinel-2 L2A Scene Classification Layer (SCL), in which observations with high cloud or shadow probability are down-weighted during the modelling process. This ensures that only high-quality observations contribute meaningfully to the fitted time series.

![The image comprises four sub-panels (a, b, c, d), each presenting a thematic map of a specific geographic area, likely an Urban Atlas tile. All panels share a common colour legend located to the right of panel (d), indicating five classes: red (value 20), orange (value 53), light yellow (value 85), light green (value 118), and dark green (value 150). The values are unitless. Each panel shows a roughly rectangular region with an irregular, jagged boundary on its right and bottom edges, consistent with a geographic tile boundary. A prominent straight diagonal line divides the internal area of interest within each panel into two distinct sections, visually comparing two different geospatial datasets or processing results for the same location. \* \*\*Panel (a):\*\* The left section shows a highly textured pattern in orange and light yellow. The right section, beyond the diagonal line, is predominantly solid red, adjacent to the jagged boundary, with a lighter red background. \* \*\*Panel (b):\*\* The left section displays a smoother, less textured pattern compared to panel (a), still primarily in orange and light yellow. The right section remains predominantly red. \* \*\*Panel (c):\*\* The left section continues to show patterns in orange and light yellow, but the right section has transitioned to a blend of light yellow and light green. \* \*\*Panel (d):\*\* The left section retains orange and light yellow hues. The right section now appears predominantly in dark green and light green, indicating a significant shift towards higher values according to the legend, suggesting an increase in the measured parameter. The progression from panel (a) to (d) likely illustrates different outputs or processing stages related to urban change detection, possibly integrating Very High-Resolution (VHR) imagery and Sentinel-2 time series data, as mentioned in the surrounding context. The diagonal split serves to highlight differences or comparisons between data sources or methodologies within the specified tile.](products_DOCS_CLMS_UA2021_LULC_ATBD_v1-media/img-368bd0a7ba855372998b556a130d1a530b822bc1.png)

Figure 2: Number of observations for Sentinel-2 as extracted from the Sen2Cor scene classification map:  
...
#### 3.4.1.1 Change detection methodology based on Sentinel-2 (S2)
...
\\\mathrm{RMSD}=\sqrt{\frac{1}{2\pi}\int\_{0}^{2\pi}(f\_{1}(t)-f\_{2}(t))^{2}~d t}\\

This expresses the average squared difference between two continuous NDVI curves (modelled by the harmonic curves f₁ and f₂ over a full annual cycle.
...
#### 3.4.1.2 Change detection methodology based on VHR
...
Finally, after several iterations and analysis of outputs, the methodology was adapted by giving priority to the **PCA approach**. The SFA results required a heavy and lengthy computing process, and results were not fully satisfactory, showing a high number of commission errors ([Figure 4](#fig-figure4)).

![This image sequence illustrates the detection of land cover change in Vienna, Austria, using Root Mean Square Deviation (RMSD) normalized by land cover class. It features three false-colour composite satellite imagery panels showing the same geographic area, with white lines outlining individual parcels. The \*\*left panel\*\* displays the initial state of the area, where vegetation appears in red. Key land cover types are indicated by yellow CLC+ Backbone codes: '11100' (Continuous urban fabric) in the top-middle, '11210' (Discontinuous urban fabric) below it, '23000' (Pastures) in the large central parcel, and two '11230' (Construction sites) in the lower section. The \*\*middle panel\*\* shows a later state of the same area. A significant change is visible in the central parcel previously classified as '23000' (Pastures), where the red vegetation has been replaced by bright white/cyan tones, indicating new construction or bare ground. The \*\*right panel\*\* is a heatmap of the normalized RMSD (RMSD_norm), which highlights pixels where observed change is significantly greater than what is typical for that land cover type. The colour gradient ranges from dark blue (lowest change) through light blue, green, and yellow, to dark red (highest change). The central parcel, where substantial land cover conversion occurred, shows a high concentration of yellow, orange, and dark red pixels, indicating a strong signal of significant change. Minor change signals (yellow pixels) are also visible in other parcels.](products_DOCS_CLMS_UA2021_LULC_ATBD_v1-media/img-09eb2e9f24c74dfd56b196c98442d476880fe8f9.png)

Figure 4: Change detection heatmap derived from VHR data (left: VHR2018 and UA2018 product; centre: VHR2021; right: Change heatmap where changes are highlighted in yellow/red) (location: Madrid, Spain)

#### 3.4.1.3 Production of change candidate layer
...
#### 3.4.1.4 Production of change layer 2018-2021 and extraction of revised 2018 layer
...
#### 3.4.1.5 Production of the status layer 2021
...
#### 3.4.1.6 Generation of Green Urban Areas (GUA) “public vs private” label
...
#### 3.4.1.7 Integration of the GUA codes
...
#### 3.4.1.9 Preparation of associated documents
...
## 4.1 Quality assurance and quality control framework
...
#### 4.1.0.1 Topological consistency
...
#### 4.1.0.4 Conceptual consistency
...
## 4.2 Thematic quality and accuracy assessment
...
### 4.2.2 Sampling and response design
...
### 4.2.3 Thematic accuracy and statistical analysis
...
## 5.1 Technical issues linked to the UA2021 LC/LU production
...
## 5.2 Technical issues linked to the GUA classification
...
# 8 Annex 1 - Green Urban Areas queries
...
# 9 Annex 2 - UA LC/LU Nomenclature
