...
This Algorithm Theoretical Basis Document details the theoretical and practical foundations underpinning the European Ground Motion Service (EGMS) product generation. It elucidates core SAR interferometric principles, multi-interferogram processing techniques, and the distinct methodologies employed by the contributing InSAR Processing Entities. The document thoroughly describes the derivation of Basic (L2a), Calibrated (L2b), and Ortho (L3) InSAR products, including their calibration using a GNSS reference frame and decomposition into vertical and east-west displacements, along with data update strategies.
...
European Ground Motion Service (EGMS), SAR interferometry (InSAR), Ground motion measurement, Persistent Scatterer Interferometry (PSI), Sentinel-1 data processing, GNSS reference frame calibration, Displacement time series, Multi-interferogram techniques, Atmospheric Phase Screen (APS), Phase unwrapping, Ortho product decomposition, Wide Area Processing (WAP)
...
\\ \max_i \left\\ \sum\_{j=1}^{N} \left( 1 - \frac{\lvert B_n^{ij} \rvert}{B_c} \right)\cdot{e^{-\lvert B_t^{ij} \rvert/\tau}} \right\\ \\
...
![This diagram illustrates a process flow for Interferometric Synthetic Aperture Radar (InSAR) data processing, specifically focusing on geometric co-registration and subsequent analysis steps. The process begins with \*\*Master Selection\*\*, which identifies a primary image from \*\*S1 Images\*\* (Sentinel-1 Synthetic Aperture Radar images). The output of this selection is a \*\*Master Image\*\*. This \*\*Master Image\*\*, along with \*\*S1 Images\*\*, \*\*Orbits\*\*, and a \*\*DEM\*\* (Digital Elevation Model), are inputs to the \*\*Geometric Co-Registration\*\* step. Following geometric co-registration, the data proceeds to \*\*Spectral Diversity\*\* analysis. The final step shown is \*\*Overlapping areas data analysis\*\*, which takes its input from the spectral diversity results. The flow indicates that master image selection is a prerequisite for the accurate geometric co-registration of SAR imagery.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-d53818566b18bd80b09acf75b51c61718394957d.png)

Figure 15 Co-registration workflow.
...
![A vector diagram illustrating the relationship between signal, noise, and phase error in Interferometric Synthetic Aperture Radar (InSAR) measurements, accompanied by a mathematical condition for phase uncertainty. The diagram displays a green vector labelled 'A (signal)' originating from the left and pointing horizontally right. A red vector labelled 'n (noise)' originates from the tip of the 'A (signal)' vector and points upwards and slightly right, terminating at a red dot. A blue vector represents the sum of the signal and noise vectors, extending from the origin of 'A (signal)' to the red dot. The angle between the 'A (signal)' vector and the blue resultant vector is labelled 'φ'. A light grey circle is centered at the tip of the 'A (signal)' vector, encompassing the red 'n (noise)' vector, visually representing the noise contribution. The accompanying mathematical expression is: 'If SNR \>\> 1 → σ_φ ~ σ_n / µ = D_A'. This indicates that if the Signal-to-Noise Ratio (SNR) is much greater than 1, then the standard deviation of the phase (σ_φ) is approximately equal to the ratio of the standard deviation of the noise (σ_n) to the mean signal amplitude (µ), with this ratio also being denoted as D_A.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-f6d1c5f382a7c0caf91add557b6f3a361ef91189.png)

Figure 19 Model behind the use of the amplitude dispersion index (DA) as a proxy for phase stability. The complex noise is supposed to be circular and Gaussian having the same standard deviation \\\sigma_n\\ for both the real and the imaginary component. \\\mu\\ represents the incoherent average of all the signal vectors A. If the signal-to-noise ratio is well above the value of 1, the ratio between \\\sigma_n\\ and \\\mu\\ is a good estimator of the phase dispersion \\\sigma\_{\phi}\\).
...
\\ \gamma_t \triangleq \frac{1}{N} \sum\_{j=1}^N e^{i \varphi_j} \\\\\Rightarrow\\\\ \mathbb{E}\[\gamma_t\] = e^{-\frac{\sigma\_\varphi^2}{2}} \\\\\Rightarrow\\\\ \mathbb{E}\[\gamma_t\] \approx e^{-\tfrac{1}{2}\left(\tfrac{\sigma_A}{\mu_A}\right)^2} = e^{-\tfrac{1}{2} D_A^2} \\\\\Rightarrow\\\\ D_A = \sqrt{-2 \log \gamma_t} \\
...
\\ \begin{array}{r}{\hat{\gamma}\_{s}(P)=\left\|\frac{\sum\_{P\_{i}\in W}z\_{M}(P\_{i})z\_{S}^{\*}\left(P\_{i}\right)}{\sqrt{\sum\_{P\_{i}\in W}\lvert z\_{M}(P\_{i})\rvert^{2}\sum\_{P\_{i}\in W}\lvert z\_{S}\left(P\_{i}\right)\rvert^{2}}}\right\|\qquad\qquad\qquad\qquad\qquad\qquad\qquad(5)}\end{array} \\
...
\\ \begin{array}{r}{\gamma\_{\mathrm{t}}(\mathrm{P})=\left\|\frac{1}{\mathrm{N}}\\\sum\_{\mathrm{i=0}}^{\mathrm{N}}\mathrm{e}^{\mathrm{j}\varphi\_{\mathrm{i}}(\mathrm{P})}\right\|\qquad\qquad\qquad\qquad\qquad(6)}\end{array} \\

Where P represents the single pixel that the evaluation is performed upon, N is the number of images, and (\\\varphi_i\\) the different pixel phase values in the images. Since the selection of MP must be carried before the estimation and removal of atmospheric components, one can check for the phase stability of pixel P vs. the mean value of the interferometric phase \\\overline{\varphi\_{1}}\\, estimated over a suitable (local) window centred on P, where atmospheric components should be very similar:

7.  

\\ \begin{array}{r}{\widehat{\gamma\_{\mathrm{t}}}(\mathrm{P})=\left\|\frac{1}{\mathrm{N}}\\\sum\_{\mathrm{i=0}}^{\mathrm{N}}\mathrm{e}^{\mathrm{j}(\varphi\_{\mathrm{i}}(\mathrm{P})-\overline{{\varphi\_{\mathrm{i}}}})}\right\|\qquad\qquad\qquad\qquad\qquad\qquad(7)}\end{array} \\
...
Measurement points belonging to the DS family are characterized by a non-zero value of a processing parameter called “Effective Area“, corresponding to a rough estimation - without considering the local topography - of the extent of the DS in m² (usually corresponding to \>20 pixels). For more information about DS the reader should refer to (Ferretti et al., 2011).
...
\\ \max\_{v, \varepsilon} \left\| \frac{1}{N} \sum\_{i=0}^{N} e^{j \left( \Delta \varphi_i - \widehat{\Delta \varphi_i} \right)} \right\| \\
...
![A network diagram illustrating the relationships between 5 numbered nodes, labeled 1, 2, 3, 4, and 5. These nodes represent individual Persistent Scatterer (PS) candidates or pixels on a sparse grid. Lines connect various pairs of nodes, each line representing an interferometric phase difference (Δφ) between the connected candidates. Specifically, the following phase differences are depicted: Δφ₁₂, Δφ₁₄, Δφ₁₅, Δφ₂₃, Δφ₂₄, Δφ₃₄, and Δφ₃₅. This graph represents the network of available phase differences used for phase unwrapping in Persistent Scatterer Interferometry (PSI) techniques.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-d0eb30964a7703138c73975ef0b289cbe7d93e90.png)

Figure 23 A graph of MPC. Links represent phase differences between the points. Each arc is weighted with temporal coherence.
...
\\ \phi_i(P) - \phi_i(P_0) = C\_{t0} + C\_{t1} B\_{ti} + \cdots + C\_{tq} B\_{ti}^q + S_s \sin(2\pi B\_{ti}) + S_c \cos(2\pi B\_{ti}) + C\_{zi} \varepsilon\_{zi} \quad i = 1 \cdots N \\

where \\q\\ is the order of the polynomial, temporal baseline values are expressed in years and \\C\_{t0}\\, … \\S_c\\ are coefficients to be estimated. In EGMS, the availability of hundreds of Sentinel-1 acquisitions all over Europe makes it possible to avoid any major problem with overfitting.
...
\\ d\_{\alpha}(r, x) = \tau(r, x) + C\_{h} h(r, x) + C\_{r} r + C\_{x} x + C\_{0} \\ where r and x are the range and the azimuth coordinate, \\\tau\\ is the component of the extra path length due to turbulence phenomena, h is the elevation of pixel P(r, x) and \\C_h\\, \\C_r\\, and \\C_0\\ are four constants regressed from the data during the processing.
...
\\ d(t) = c\_{0} + c\_{1} t + c\_{2} t^{2} + c\_{3} t^{3} + A \cos(2 \pi t + \varphi) \\ where the unit of time is 1 year.
...
![This image displays three Sentinel-1 Interferometric Wide (IW) swath sub-images, labeled IW3 (left), IW2 (middle), and IW1 (right), illustrating the spatial arrangement of radar data bursts. Each sub-image presents a grayscale view of a terrestrial area, revealing varied land cover features such as urban textures, agricultural patterns, and water bodies. Black horizontal lines segment each IW sub-image, visually delineating individual radar bursts. IW3 consists of 7 visible horizontal bursts. IW2 contains 8 visible horizontal bursts and shows a large inland water body in its upper right region. IW1 also contains 8 visible horizontal bursts and depicts a prominent coastline along its right and bottom-right edges. The image demonstrates the structure of raw satellite data, particularly the division into bursts and contiguous swaths used in Interferometric Synthetic Aperture Radar (InSAR) processing.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-146ef7be7fbfd2e7fcf01908515d579e5ac065bf.jpg)

Figure 29 Sentinel-1, IW acquisition mode. Each image is composed by 3 main swaths, each one being a collection of 9 bursts. https://sentinels.copernicus.eu/web/sentinel/user-guides/sentinel-1-sar/acquisition-modes/interferometric-wide-swath
...
![This image is a 3D rendering of the Earth from space, illustrating satellite data acquisition coverage, likely for Interferometric Synthetic Aperture Radar (InSAR). The Earth, showing visible landmasses (primarily Africa, Europe, and parts of Asia) and oceans, is enveloped by numerous translucent, light-grey lines representing satellite orbital tracks or coverage swaths. Two small satellites are depicted in orbit, one positioned above the Northern Hemisphere and another below the Southern Hemisphere, each emitting a green, laser-like beam towards the Earth's surface, symbolising active InSAR data collection. In the bottom-right corner, a control panel is visible with the label 'Full coverage in days:'. Below this, a timeline with numbers '0 1 2 3 4 5 6' indicates the duration. A green highlighted bar extends from '0' to approximately '1', suggesting that full coverage is achieved within one day. This visualisation pertains to the processing strategies for the European Ground Motion Service (EGMS), which relies on InSAR data.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-56f1e507b27f4af0b00456ca54d2820e567d3c84.jpg)

Figure 30 Sentinel-1 constellation acquisition by relative orbit. https://sentinels.copernicus.eu/web/sentinel/missions/sentinel-1/satellite-description/orbit
...
As already reported in Section [Section 4.4.2.2](#sec-442Sidelobe-reduction-techniques) different algorithms are applied by the four IPEs to limit the impact of “ghost-MP” created by strong, dominant, PS.
...
![This diagram illustrates a data processing workflow for generating L2b and L3 products from L2a products and Global Navigation Satellite System (GNSS) model data, likely within the context of European Ground Motion Service (EGMS) Interferometric Synthetic Aperture Radar (InSAR) data calibration. The workflow begins with a \`GNSS Model\` which branches into two parallel processes: 1. \`GNSS Model Interpolation\`. 2. \`GNSS N/S Component in SAR LOS\` (Synthetic Aperture Radar Line Of Sight). The \`GNSS Model Interpolation\` output, along with \`L2a Products\`, feeds into \`Interpolated Model Projection\`. The output of \`Interpolated Model Projection\` then proceeds to \`SAR/GNSS Referencing\`. \`SAR/GNSS Referencing\` leads to a \`Compensation\` step, which also receives direct input from \`L2a Products\`. This first \`Compensation\` step produces \`L2b Products\`. The \`L2b Products\` then feed into a second \`Compensation\` step. This second \`Compensation\` step also receives input from the \`GNSS N/S Component in SAR LOS\` branch. The output of the second \`Compensation\` step feeds into \`Decomposition\`. Finally, \`Decomposition\` produces \`L3 Products\`, representing the end product of this processing chain.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-7be7b9e910835788e15dbcbd0ffa9ce75f835d7d.png)

Figure 36 L2b products calibration process.
...
\\ d=d\_{x}u\_{X}+d\_{y}u\_{y}+d\_{z}u\_{z} \\

The component of the displacement vector measured by the radar can be easily computed once we know the unit vector associated to the satellite line of sight (\\u\_{LOS}\\):

\\ d\_{LOS}=d\cdot{u\_{LOS}} \\
...
\\ \begin{bmatrix} d\_{LOS1} \\ d\_{LOS2} \\ d\_{LOS3} \end{bmatrix} = \begin{bmatrix} u\_{LOS1x} & u\_{LOS1y} & u\_{LOS1z} \\ u\_{LOS2x} & u\_{LOS2y} & u\_{LOS2z} \\ u\_{LOS3x} & u\_{LOS3y} & u\_{LOS3z} \end{bmatrix} \begin{bmatrix} dx \\ dy \\ dz \end{bmatrix} = \mathbf{U}\\ \mathbf{d} \\
...
\\ \begin{bmatrix} d\_{LOS1} \\ d\_{LOS2} \\ \vdots \\ D\_{GPSx} \\ D\_{GPSy} \\ D\_{GPSz} \end{bmatrix} = \begin{bmatrix} u\_{LOS1x} & u\_{LOS1y} & u\_{LOS1z} \\ u\_{LOS2x} & u\_{LOS2y} & u\_{LOS2z} \\ \vdots & \vdots & \vdots \\ 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{bmatrix} \begin{bmatrix} d_x \\ d_y \\ d_z \end{bmatrix} = \mathbf{U}\\\mathbf{d} \\
...
![An abstract graphic titled 'Decomposed' presents a 2x2 grid composed of four equal square cells. The top-left and bottom-right cells are filled with a light green colour, while the top-right and bottom-left cells are filled with a light orange colour. A single blue circular dot is centrally positioned within the top-left light green cell. Another single blue circular dot is centrally positioned within the bottom-right light green cell. This visual conceptually illustrates the decomposition process, where ground displacement data from European Ground Motion Service (EGMS) Calibrated (L2b) products is separated into discrete geospatial layers containing purely vertical and purely east-west displacement components, as applied in the generation of EGMS Ortho products.](products_products_European_Ground_Motion_Service_Algorithm_Theoretical_Basis_Document_v4-media/img-f33f132d1bee4450ab37f585955818f90bba441b.png)

Figure 41: To maintain reliability, decomposition is performed just on cells which contains a minimum number of MPs in both the ascending and descending orbits.
...
L3 products are derived from L2b products following the rationale and the approach described in section [Section 9](#sec-9EGMS-PRODUCTION-PROTOCOL-ORTHO-PRODUCT). As a consequence, L3 products update is automatically obtained as a consequence of the L2a and L2b one.
