...
This Product User Manual introduces the Copernicus Land Monitoring Service’s CLCplus Backbone 2023 product, a high-resolution, seamless land cover raster dataset for the entire EEA38, UK, and French DOMs. It details the 11-class land cover nomenclature, derived from Sentinel-2 time-series data at 10m spatial resolution, and explains the underlying decision tree approach. The document outlines the product’s technical specifications, methodologies, and an internal quality assessment, alongside descriptions of auxiliary layers (Data Score, Confidence, and Post-processing). This ensures users can effectively understand and utilise this essential geospatial information for diverse environmental applications.
...
CLCplus Backbone, land cover raster, Sentinel-2 time series, 10m spatial resolution, EEA38+UK, EAGLE Land Cover Components, Temporal Convolutional Neural Network, thematic accuracy, coastal seawater buffer, Raster Data Score Layer, Raster Confidence Layer, LULUCF
...
- A separate technical class has been introduced for the seawater areas in the buffer around the production AOI since the 2021 production (see [Section 7](#sec-product-methodology-and-workflow) and AD-1).
...
- Since the 2023 reference year, the file naming has been adapted as described in [Section 6.1](#sec-CLCplus-Backbone-2023-Raster)
...
From the above, CLCplus Backbone enables applications in the following domains – among others (see also [Section 5.2](#sec-thematic-application-areas)):
...
- **Habitat Assessment**: Conservationists and ecologists will find it useful to apply CLCplus Backbone to assess the distribution of certain habitats that can be related to land cover, helping to monitor their health and biodiversity.
...
The CLCplus Backbone 2023 product comprises an updated layer of the main land cover raster layer, as well as three auxiliary layers (see [Figure 1](#fig-figure1)).

![This diagram illustrates the structure and accompanying layers of the Copernicus Land Monitoring Service (CLMS) CLCplus Backbone (BB) Raster Update product for 2023. The primary product, the CLCplus BB Raster Update 2023, is characterised by 11 land cover classes and a 10-metre spatial resolution. Associated with this main product are three complementary layers: a Confidence Layer, a Data Score Layer, and a Post-Processing Layer.](products_products_CLCplus_Backbone_2023_PUM_v1-media/img-3ae7a950b89b88ab5164a45a96c174e993f664d9.png)

Figure 1: Overview of CLCplus Backbone layers produced for the reference year 2023
...
- Raster classification file: *CLMS_CLCPLUS_RAS_S2023_R10m_E44N36_03035_V01_R00.tif*

- Attribute table file: *CLMS_CLCPLUS_RAS_S2023_R10m_E44N36_03035_V01_R00.tif.aux.xml*
...
Table 5: Technical specifications of the CLCplus Backbone Raster Confidence Layer
...
The product specifications for the CLCplus Backbone Raster Post-Processing Layer (RASPL) are summarized in Table 6.
...
- Pelletier, C., Webb G.I. & Petitjean, F. (2019). Temporal Convolutional Neural Network for the Classification of Satellite Image Time Series. Remote Sensing 11, no. 5: 523. [https://doi.org/10.3390/rs11050523](https://www.mdpi.com/2072-4292/11/5/523)
