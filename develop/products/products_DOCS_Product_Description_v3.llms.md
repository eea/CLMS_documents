...
This document provides a comprehensive description of the European Ground Motion Service (EGMS) products, specifically detailing the Basic, Calibrated, and Ortho displacement datasets. It outlines the technical specifications for each, including spatial and temporal resolutions, accuracy metrics, and file formats such as GeoTIFF and CSV. The manual clarifies the processing approaches, highlighting the use of InSAR data calibrated with GNSS observations, and explains the derivation of absolute vertical and east-west ground motion components. This guide is essential for scientists, regulators, and data engineers to understand EGMS product characteristics, ensuring effective utilisation for ground motion analysis across Europe.
...
European Ground Motion Service (EGMS), InSAR displacement data, GNSS-based reference model, Ground motion monitoring, ETRS89-LAEA projection, Spatial and temporal resolution, Mean velocity resolution, Displacement standard deviation, Sentinel-1 satellite imagery, Data product specifications, Measurement Point density
