...
This mapping guide serves as a Product User Manual for the Copernicus Urban Atlas, detailing the Land Use/Land Cover (LU/LC) product, Street Tree Layer (STL), and Digital Height Model (DHM) for Functional Urban Areas across Europe. It outlines specifications, methodologies, and quality control procedures for mapping products from 2006, 2012, and 2018. The document focuses on visual interpretation of Very High Resolution (VHR) data, integrated with navigation and imperviousness data, ensuring consistent nomenclature, spatial resolution, and thematic accuracy for urban and surrounding areas.
...
Urban Atlas, land use/land cover mapping, Functional Urban Areas, Street Tree Layer, Digital Height Model, Very High Resolution imagery, change detection, urban monitoring, thematic accuracy, minimum mapping unit, CORINE Land Cover, imperviousness degree
...
![A simple two-row table defining a land cover classification code. The header row is labelled 'CODE_yyyyy'. The data row contains the value '11210\*'. This code likely represents a specific land use/land cover class, such as 'Industrial or commercial units' based on the CORINE Land Cover (CLC) nomenclature for urban areas, as hinted by the document's surrounding context.](products_DOCS_Land_Cover-Land_Use_and_Street_Tree_Layer_2012_and_2018_PUM_v6-media/image1.jpg)

Example provided of the number for UA class 1.1.2.1

![\| CODE \| Legend code \| \|---\|---\| \| yyyy \| Reference year (e.g. 2006, 2012 or 2018) \| This table defines the code \`yyyy\` as a representation for a 'Reference year' within technical documentation, providing examples such as 2006, 2012, and 2018.](products_DOCS_Land_Cover-Land_Use_and_Street_Tree_Layer_2012_and_2018_PUM_v6-media/image2.jpg)
...
![This diagram illustrates a mapping rule for a linear feature with varying width and a maximum length. The feature is bounded by two black lines on a light yellow background. At its widest points, indicated by vertical double-headed arrows on the left and right, the feature has a width of 10 m. The central portion of the feature narrows significantly. A horizontal double-headed arrow below the feature indicates a length of '≤ 50 m', encompassing the section from where the feature begins to narrow to where it regains its 10 m width. This geometric configuration is labelled as 'Mapped as 1.2.2.3', suggesting a classification code within a land cover mapping system like CORINE Land Cover (CLC).](products_DOCS_Land_Cover-Land_Use_and_Street_Tree_Layer_2012_and_2018_PUM_v6-media/image36.png)
