...
- [<span class="toc-section-number">2</span> Upcoming](#upcoming)
- [<span class="toc-section-number">3</span> CLMS Product on
...
<div style="background:#fff3e0;border:1px solid #ffcc80;border-radius:6px;padding:12px;margin:12px 0">
  ...
  ${odataDaily.daily_avg ? htl.html `<div style="margin:4px 0;color:#888;font-size:0.85em">Daily average: ${odataDaily.daily_avg.toLocaleString()} new files/day</div>` : ""}
  ${onboardingPlot}
</div>` : ""}
`
```
...
productGroupsTable = {
  ...
  catStats.forEach(cat => {
    ...
    cat.groups.forEach(g => {
      ...
      const lpOffCdse = (g.lp_datasets || []).filter(lp => !lp.on_cdse && activeComponents.has(lp.component));
