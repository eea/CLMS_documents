...
- [<span class="toc-section-number">1</span> Migration
  Statistics](#migration-statistics)
- [<span class="toc-section-number">2</span> CLMS Product on
  CDSE](#clms-product-on-cdse)
- [<span class="toc-section-number">3</span> On CDSE
  soon](#on-cdse-soon)
- [<span class="toc-section-number">4</span> Services](#services)
- [<span class="toc-section-number">5</span> Methodology](#methodology)
...
This table shows the full, currently active CLMS portfolio and each
product’s migration status to the CDSE.
...
> *Details to be provided — placeholder for upcoming CLMS products
> planned for CDSE ingestion.*

Preparing dataset descriptions, expected timelines, and service coverage
for the next batch of products to be onboarded.
...
- [CDSE
  Documentation](https://documentation.dataspace.copernicus.eu/Data/CopernicusServices/CLMS.html)
...
- **STAC**: Fuzz-matched by checking if CDSE dataset ID (minus
  `_cog`/`_nc` suffix) appears in STAC collection IDs
