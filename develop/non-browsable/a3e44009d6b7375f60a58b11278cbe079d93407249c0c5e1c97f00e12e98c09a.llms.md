...
CLMS product migration, Copernicus Data Space Ecosystem (CDSE), CLMS data availability, S3 data access, STAC collections, OpenEO integration, OData API, Sentinel Hub data, product onboarding, data ingestion metrics, CLMS product groups
...
The CLMS portfolio is composed of datasets in product groups (current selection). Of these, the following are available on the CDSE:

| Metric          | Value        |
|-----------------|--------------|
| CLMS in S3      |  dataset IDs |
| CLMS via OData  |  files       |
| CLMS via STAC   |  collections |
| CLMS via OpenEO |  collections |
